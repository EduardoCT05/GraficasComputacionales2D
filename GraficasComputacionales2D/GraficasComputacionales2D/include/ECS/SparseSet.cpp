...
namespace ECS {
    ...
    // SparseSet
    // Estructura de datos para asociar entidades a índices densos de forma eficiente.
    //
    ...
    public:
        ...
        // Retorna `true` si la entidad está en el set.
        ...
        // Referencia constante al array denso de EntityIDs activos.
        [[nodiscard]] const std::vector<EntityId>& GetEntities() const noexcept {
            return m_dense;
        }

        //  Elimina una entidad usando swap-with-last para mantener densidad.
        //
        // Las subclases deben sincronizar sus propios arrays *antes* de
        // llamar a esta base (ver `ComponentPool::Remove`).
        //  entity Entidad a eliminar. Debe estar en el set.
    ...
    protected:
        ...
        EntityIndex InsertEntity(EntityId entity) {
            ...
            assert(m_sparse[sparseIdx] == INVALID &&
