...
Window::Window(int width, int height, const std::string& title) {
	...
	if (m_window) {
		...
		m_baseViewSize = sf::Vector2f(static_cast<float>(width), static_cast<float>(height));
...
void Window::handleResize(const sf::Vector2u& size) {
	...
	m_baseViewSize = sf::Vector2f(static_cast<float>(size.x), static_cast<float>(size.y));
	m_view.setSize(m_baseViewSize);
}

void Window::applyCameraView(const sf::Vector2f& position, float zoom, float rotationDeg) {
	// 1. Centramos la cámara en la posición de la entidad
	m_view.setCenter(position);

	// 2. Aplicamos el zoom usando el tamaño base
	m_view.setSize(sf::Vector2f(m_baseViewSize.x * zoom, m_baseViewSize.y * zoom));
