...
CShape::CShape(ShapeType shapeType)
    : m_shapeType(shapeType), m_shape(createShape(shapeType)) {
}
...
std::unique_ptr<sf::Shape> CShape::createShape(ShapeType shapeType) {
    ...
    case RECTANGLE: {
        auto rectangle =
            std::make_unique<sf::RectangleShape>(sf::Vector2f(120.f, 60.f));
    ...
    case TRIANGLE: {
        ...
        triangle->setPoint(2, sf::Vector2f(50.f, 86.6f));
    ...
    case LINE: {
        auto line =
            std::make_unique<sf::RectangleShape>(sf::Vector2f(100.f, 5.f));
